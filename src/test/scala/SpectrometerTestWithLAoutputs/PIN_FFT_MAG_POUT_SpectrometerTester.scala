...
(
 ...
 // val complexOut = realSeq.zip(imagSeq).map { case (real, imag) => Complex(real, imag) }
  ...
  // Plot scala FFT
  SpectrometerTesterUtils.plot_fft(inputData = scalaPlot, plotName = "Scala FFT", fileName = "SpectrometerTestWithLAoutputs/pin_fft_mag_pout_scalaFFT.pdf")
  // Plot input data
  SpectrometerTesterUtils.plot_data(inputData = inData, plotName = "inData", fileName = "SpectrometerTestWithLAoutputs/pin_fft_mag_pout_inData.pdf")
  // Plot accelerator data -> HERE PERHAPS ERROR -> should plot only imag part
  //SpectrometerTesterUtils.plot_fft(inputData = chiselFFTForPlot, plotName = "PIN -> FFT -> MAG -> POUT", fileName = "SpectrometerTestWithLAoutputs/pin_fft_mag_pout.pdf")
  SpectrometerTesterUtils.plot_data(inputData = imagSeq.map(c => c.toInt), plotName = "PIN -> FFT -> MAG -> POUT", fileName = "SpectrometerTestWithLAoutputs/pin_fft_mag_pout.pdf")
