...
(
  ...
  SpectrometerTesterUtils.plot_data(inputData = imagSeq.map(c => c.toInt), plotName = "PLFG -> NCO -> FFT -> MAG -> POUT", fileName = "SpectrometerTestWithLAoutputs/plfg_nco_fft_mag_pout/data.pdf")

  // Write output data to text file
  val file = new File("./test_run_dir/SpectrometerTestWithLAoutputs/plfg_nco_fft_mag_pout/data.txt")
